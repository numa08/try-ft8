...
| AC-11 | デコード結果のテキスト表示                | **HARNESS** + MANUAL | `test/unit/ft8-roundtrip.test.ts`(WASM 同梱後に有効化)+ 実機音響結合は手動             |
...
SPEC 6章に従い、AC-11 は以下の治具を用意して自動化する。WASM 同梱前は当該テストを `test.todo` / `skip` で保留し、
握りつぶさずトレーサビリティに「治具待ち」と明記する。

1. **エンコード↔デコード往復**: `encode("CQ DE ABC")` で生成した音声サンプルを `decode()` に通し、元テキストが復元されること。
2. **ゴールデン音声**: 事前生成した既知の `.wav`(`test/fixtures/*.wav`)をデコードし、期待テキストに一致すること。

インターフェース境界(実装で埋める):

```ts
// src/audio/ft8.ts (WASM ラッパ)
export function encode(text: string, offsetHz: number): Float32Array // 音声サンプル(モノラル)
export function decode(samples: Float32Array): { text: string; offsetHz: number }[]
```
